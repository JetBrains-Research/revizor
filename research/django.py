...
def traverse_python_methods(repository_root: str) -> Generator[Method, None, None]:
        ...
        for file in files:
            if file.endswith('.py'):
                ...
                with open(path_to_python_file, 'r') as f:
                    src = f.read()
                methods: List[Method] = GitAnalyzer._extract_methods(path_to_python_file, src)
...
if __name__ == '__main__':
    ...
    results = {}
    for method in traverse_python_methods(repository_root=os.path.join(config.REPOSITORIES_ROOT, 'django')):
