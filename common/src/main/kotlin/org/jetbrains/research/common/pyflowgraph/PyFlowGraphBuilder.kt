...
import org.jetbrains.research.common.pyflowgraph.models.*
import org.jetbrains.research.common.pyflowgraph.postprocessing.DependenciesResolver
import org.jetbrains.research.common.pyflowgraph.postprocessing.TransitiveClosureBuilder
...
class PyFlowGraphBuilder {
        ...
        if (flowGraph.entryNode == null) {
            ...
            for (statement in node.statementList.statements) {
                ...
                if (statementFlowGraph == null) {
                    logger.warn("Unable to build pfg for $statement, skipping...")
                    continue
                }
    ...
    private fun visitFunctionDefParameters(node: PyCallable): List<PyFlowGraph> {
        ...
        if (parametersFlowGraphs.any { it == null }) {
            logger.warn("Unsupported parameters in function definition, skipping them...")
        }
    ...
    private fun visitWhile(node: PyWhileStatement): PyFlowGraph {
        ...
        return whileFlowGraph ?: throw GraphBuildingException("Unable to build pfg for <${node.text}>")
    }
    ...
    private fun visitControlNodeBodyHelper(
        ...
        for (statement in statements) {
            ...
            try {
                ...
                if (statementFlowGraph == null) {
                    logger.warn("Unable to build graph for $statement inside ${controlNode.psi}, skipping...")
                    continue
                }
